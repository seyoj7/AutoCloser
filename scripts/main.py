...
def _pick_and_invoice(lead: dict) -> bool:
    ...
    if NO_INPUT:
        choice = "2"
        print(f"  [Auto] Selected default service ID: {choice}")
    else:
    ...
    for lead in leads:
        ...
        # meeting_booked -> ask user if meeting is done, then optionally invoice
            ...
            if NO_INPUT:
                ...
                print(f"  [Auto] Marking meeting as completed for {lead['company']}")
            else:
            ...
            if done == "y":
                ...
                if NO_INPUT:
                    send_invoice = "y"
                    print(f"  [Auto] Creating invoice for {lead['company']}")
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SalesHermes Pipeline")
